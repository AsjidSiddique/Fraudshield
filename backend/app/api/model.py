...
def get_samples_preview(rows: int = 5) -> dict:
    """A small, real preview of the bundled sample_transactions.csv — shown on the Batch
    Analysis page so people know exactly what a valid upload should look like before they
    try to build or trim their own CSV."""
    import pandas as pd

    if not settings.SAMPLE_TRANSACTIONS_PATH.exists():
        raise HTTPException(status_code=404, detail="Sample transaction file was not bundled with this deployment.")

    tx = pd.read_csv(settings.SAMPLE_TRANSACTIONS_PATH)
    n = max(1, min(rows, len(tx)))
    return {
        "columns": REQUIRED_COLUMNS,
        "rows": tx[REQUIRED_COLUMNS].head(n).to_dict(orient="records"),
        "total_rows_in_file": len(tx),
    }


@router.get("/samples/csv")
def download_samples_csv():
    """Serves the bundled sample_transactions.csv verbatim so it can be downloaded and
    re-uploaded on the Batch Analysis page — a small (~13 KB), correctly formatted file
    that will pass validation, unlike the full 150 MB raw Kaggle dataset."""
    if not settings.SAMPLE_TRANSACTIONS_PATH.exists():
        raise HTTPException(status_code=404, detail="Sample transaction file was not bundled with this deployment.")
    return FileResponse(
        path=settings.SAMPLE_TRANSACTIONS_PATH,
        media_type="text/csv",
        filename="fraudshield_sample_transactions.csv",
    )

...
import { ShieldAlert, ShieldCheck, Copy, SlidersHorizontal, ArrowRight } from "lucide-react";
...
export function ResultCard({ result, showExplainLink = false }: { result: PredictionResult; showExplainLink?: boolean }) {
      ...
      <Card className={isFraud ? "glow-fraud border-fraud/30" : "glow-legit border-legit/30"}>
        <CardContent className="flex flex-col gap-6 pt-6">
          ...
          <p className="text-xs text-muted-2">
            Risk level is a presentation layer derived directly from the model&apos;s predicted
            probability relative to the decision threshold — it is not a calibrated estimate of
          ...
          {showExplainLink && (
            <Link
              href="/explainability"
              className="link-underline inline-flex w-fit items-center gap-1.5 text-sm text-accent"
            >
              View SHAP explanation <ArrowRight className="h-3.5 w-3.5" />
            </Link>
          )}
        </CardContent>
      </Card>
